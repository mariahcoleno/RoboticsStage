...
class RoboticArmEnv(gym.Env):
    ...
    def _apply_action(self, action):
        ...
        dy = dy * 0.03
        dz = dz * 0.03
        gripper_action = gripper_action * 0.05
    ...
    def step(self, action):
        ...
        if dist_cube_target < 0.15:
            # Near target, give extra reward - REDUCED
            reward += 5.0  # Reduced from 10.0 to avoid lingering
            
            # If close to target and gripper open, give large success reward
            if dist_cube_target < 0.1 and self.gripper_state == 0:
                reward += 15000.0  # Increased from 100.0 to strongly encourage releasing
        ...
        # Penalty for holding the cube at the target without releasing - INCREASED
        if dist_cube_target < 0.1 and self.gripper_state == 1:
            reward -= 500.0  # Increased from 100.0 to strongly discourage holding at target
        ...
        truncated = self.total_steps >= 200  # Increased from 100 to 200 to give more time to learn
...
def main():
    ...
    if args.mode == "train":
        ...
        logger.info(f"Starting training with args: mode={args.mode}, timesteps={args.timesteps}, checkpoint={args.checkpoint}")
        ...
        if checkpoint_path and not checkpoint_path.endswith('.zip'):
            checkpoint_path = checkpoint_path + '.zip'
            logger.info(f"Appended .zip to checkpoint path: {checkpoint_path}")

        logger.info(f"Checking if checkpoint exists at: {checkpoint_path}")
        if checkpoint_path and os.path.exists(checkpoint_path):
            logger.info(f"Checkpoint exists, attempting to load: {checkpoint_path}")
            try:
                model = PPO.load(checkpoint_path, env=env)
                model.ent_coef = 0.01  # Already set to 0.01, which worked well
                logger.info("Successfully loaded checkpoint")
            except Exception as e:
                logger.error(f"Failed to load checkpoint {checkpoint_path}: {e}")
                raise
        else:
            logger.info(f"No checkpoint found at {checkpoint_path}, starting new model")
            ent_coef = 0.01  # Already set to 0.01, which worked well
            model = PPO("MlpPolicy", env, verbose=1, tensorboard_log="./logs", ent_coef=ent_coef, learning_rate=0.0001, batch_size=64, n_steps=4096)

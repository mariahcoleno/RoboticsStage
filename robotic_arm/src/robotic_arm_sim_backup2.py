...
class RoboticArmEnv(gym.Env):
    def __init__(self, use_gui=False):
        ...
        self.prev_cube_z = None  # For Z-axis shaping reward
        
        # More simulation steps for better stability
        self.sim_steps_per_action = 20  # Already set correctly
    ...
    def _apply_action(self, action):
        ...
        dx = dx * 0.03  # Already reduced
        dy = dy * 0.03  # Already reduced
        dz = dz * 0.03  # Already reduced
    ...
    def step(self, action):
        ...
        # UPDATED REWARD FUNCTION TO ENCOURAGE RELEASING
        reward = 0.0
        done = False
        
        # Base penalty for time
        reward -= 1.0  # Increased from 0.1 to encourage faster action
        
        # Stage 1: Approaching the cube
        if not self.ever_grasped:
            # Reward for getting close to the cube
            reward -= 5.0 * dist_ee_cube  # Scaled up from 5.0
            
            # Reward for closing gripper when close to cube
            if dist_ee_cube < grasp_threshold and self.gripper_state == 1:
                reward += 5.0  # Scaled up from 5.0
                
            # Shaping reward for approaching cube
            if self.prev_dist_ee_cube is not None:
                approach_reward = 10.0 * (self.prev_dist_ee_cube - dist_ee_cube)  # Reduced from 20.0
        ...
        if self.grasping:
            ...
            # Reward for moving cube toward target
            reward -= 15.0 * dist_cube_target  # Reduced from 30.0
            
            # Shaping reward for moving toward target
            if self.prev_cube_target_dist is not None:
                target_approach_reward = 25.0 * (self.prev_cube_target_dist - dist_cube_target)  # Reduced from 50.0
                reward += target_approach_reward
            
            # NEW: Shaping reward for lowering cube Z position
            cube_z = self.cube_pos[2]
            target_z = self.target_pos[2]
            if self.prev_cube_z is not None:
                z_approach_reward = 20.0 * (self.prev_cube_z - cube_z)  # Reward for lowering Z (positive when Z decreases)
                reward += z_approach_reward
            # Also add a direct penalty for Z distance
            z_dist = abs(cube_z - target_z)
            reward -= 10.0 * z_dist
        
        # Stage 3: Releasing at target
        if dist_cube_target < 0.15:
            # Near target, give extra reward
            reward += 5.0  # Reduced from 10.0 to avoid lingering
            
            # If close to target and gripper open, give large success reward - RELAXED THRESHOLD
            if dist_cube_target < 0.12 and self.gripper_state == 0:  # Relaxed from 0.1 to 0.12
                reward += 10000.0  # Scaled up from 100.0 to strongly encourage releasing
                logger.info(f"Timestep {self.total_steps}: Released cube")
        ...
        if not self.grasping and self.ever_grasped and dist_cube_target > 0.15:
            reward -= 10.0  # Reduced from 20.0
        
        # Penalty for holding the cube at the target without releasing - RELAXED THRESHOLD
        if dist_cube_target < 0.12 and self.gripper_state == 1:  # Relaxed from 0.1 to 0.12
            reward -= 500.0  # New penalty to strongly discourage holding at target
        
        # Additional time penalty after grasping to encourage faster completion
        if self.ever_grasped:
            reward -= 5.0  # New penalty to speed up task completion
        
        # Update previous values for next step
        self.prev_dist_ee_cube = dist_ee_cube
        self.prev_cube_target_dist = dist_cube_target
        self.prev_cube_z = self.cube_pos[2]  # Update Z position for shaping reward
        ...
        # Episode length is already set to 400 steps
    ...
    def reset(self, seed=None, options=None):
        ...
        self.prev_cube_target_dist = None
        self.prev_dist_ee_cube = None
        self.prev_cube_z = None  # Reset Z tracking
...
class LoggingCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.num_timesteps % self.log_freq == 0:
            ...
            if info.get("dist_cube_target", float('inf')) < 0.12 and info.get("gripper_state", 0) == 0:  # Updated threshold
...
def main():
    ...
    if args.mode == "train":
        ...
        if checkpoint_path and os.path.exists(checkpoint_path):
            ...
            model.ent_coef = 0.05  # Increased from 0.001 to encourage more exploration
        else:
            ent_coef = 0.05  # Increased from 0.001 to encourage more exploration
        ...
        model.learn(total_timesteps=args.timesteps, callback=[checkpoint_callback, logging_callback], reset_num_timesteps=False)
        ...
        for episode in range(5):
            ...
            for step in range(400):
                ...
                if info["dist_cube_target"] < 0.12 and info["gripper_state"] == 0:  # Updated threshold
